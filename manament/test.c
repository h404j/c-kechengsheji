...
{
    printf("学 号\t姓名\t   班   级\t学期\t英语\t数学\tC语言\t总分\n"); //打印表头
...
void findclassstu(struct manament *m, char class[20])
{
    printf("学 号\t姓名\t   班   级\t学期\t英语\t数学\tC语言\t总分\t排名\n"); //打印表头
    int ii = 0;
...
void savestu(struct manament *m) //完成人：王豪杰
{
    ...
    {
        if ((fp = fopen("managent1.txt", "w")) == NULL) //判断文件是否打开失败
        {
    ...
    {
        if ((fp = fopen("managent1.txt", "a")) == NULL) //判断函数是否打开失败
        {
            printf("文件打开失败"); //文件打开失败提示
            exit(0);                //退出系统
        }
...
{
    FILE *fp;                                      //文件指针
    if ((fp = fopen("managent1.txt", "w")) == NULL) //判断是否打开文件
    {
        printf("文件打开失败"); //文件打开失败提示
        exit(0);                //退出系统
    }
...
{
    FILE *fp;                                      //定义稳健指针
    if ((fp = fopen("managent1.txt", "r")) == NULL) //判断是否打开
    {
        FILE *fp1;                                      //文件指针
        if ((fp1 = fopen("managent1.txt", "w")) == NULL) //判断是否打开文件
        {
            printf("文件打开失败"); //文件打开失败提示
            exit(0);                //退出系统
        }
